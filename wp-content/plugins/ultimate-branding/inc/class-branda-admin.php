	...
	class Branda_Admin extends Branda_Base {
		...
		private function load_permissions() {
			if ( !class_exists( 'Branda_Permissions' ) ) {
				ub_load_single_module( 'utilities/permissions.php' );
			}
		...
		private function check_user_access() {
			...
			$permissions = Branda_Permissions::get_instance();
			$user_ids = $permissions->get_allowed_user_ids();

			if ( in_array( $user_id, $user_ids, true ) ) {
				return true;
			}

			$allowed_roles = $permissions->get_allowed_roles();
