	...
	public function wcfmmp_wcfm_inactive_notice() {
		...
		?>
			<div class="notice wcfm_addon_inactive_notice_box wcfmmp_addon_inactive_notice_box" id="wcfm-groups-sttafs-notice">
				...
				<?php echo $offer_msg; ?>
				<a href="<?php echo admin_url( 'plugin-install.php?tab=search&s=wc+frontend+manager' ); ?>" class="button button-primary promo-btn" target="_blank"><?php _e( 'GET IT NOW', 'wc-multivendor-marketplace' ); ?></a>
			</div>
		<?php
	}
  ...
  function wcfmmp_store_product_data_save( $product_id ) {
  	...
  	if( $wcfmmp_store ) {
		...
		if( isset( $_POST['commission'] ) && !empty( $_POST['commission'] ) ) {
			update_post_meta( $product_id, '_wcfmmp_commission', esc_sql($_POST['commission']) );
		}
  }
  ...
  function wcfmmp_store_variation_settings_fields( $loop, $variation_data, $variation ) {
	...
	function wcfmmp_store_save_variation_settings_fields( $variation_id ) {
		...
		if( isset( $_POST['vendor_commission_mode'][$variation_id] ) ) {
			$variation_commission_data['commission_mode'] = esc_sql($_POST['vendor_commission_mode'][$variation_id]);
		}
		if( isset( $_POST['vendor_commission_percent'][$variation_id] ) ) {
			$variation_commission_data['commission_percent'] = esc_sql($_POST['vendor_commission_percent'][$variation_id]);
		}
		if( isset( $_POST['vendor_commission_fixed'][$variation_id] ) ) {
			$variation_commission_data['commission_fixed'] = esc_sql($_POST['vendor_commission_fixed'][$variation_id]);
		}
	...
	public function wcfmmp_save_product_cat_commission_fields( $term_id, $tt_id = '', $taxonomy = '' ) {
		if( isset( $_POST['commission'] ) && 'product_cat' === $taxonomy ) {
			if( function_exists( 'update_term_meta' ) ) {
				update_term_meta( $term_id, '_wcfmmp_commission', esc_sql($_POST['commission']) );
			} else {
				update_metadata( 'woocommerce_term', $term_id, '_wcfmmp_commission', esc_sql($_POST['commission']) );
			}
		}
	}
