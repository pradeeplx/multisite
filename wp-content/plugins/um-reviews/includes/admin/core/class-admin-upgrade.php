<?php
namespace um_ext\um_reviews\admin\core;

// Exit if accessed directly.
if ( ! defined( 'ABSPATH' ) ) exit;

if ( ! class_exists( 'um_ext\um_reviews\admin\core\Admin_Upgrade' ) ) {
	/**
	...
	class Admin_Upgrade {
		...
		function __construct() {
			...
			$um_reviews_last_version_upgrade = get_option( 'um_reviews_last_version_upgrade' );

			if ( ! $um_reviews_last_version_upgrade || version_compare( $um_reviews_last_version_upgrade, um_reviews_version, '<' ) )
		...
		public function packages() {
			...
			$um_reviews_last_version_upgrade = get_option( 'um_reviews_last_version_upgrade' );
			$um_reviews_last_version_upgrade = ! $um_reviews_last_version_upgrade ? '0.0.0' : $um_reviews_last_version_upgrade;

			foreach ( $this->update_versions as $update_version ) {

				if ( version_compare( $update_version, $um_reviews_last_version_upgrade, '<=' ) )
					continue;

				if ( version_compare( $update_version, um_reviews_version, '>' ) )
					continue;
				...
				if ( file_exists( $file_path ) ) {
					...
					update_option( 'um_reviews_last_version_upgrade', $update_version );
				}
			}
