...
if ( ! class_exists( 'Branda_Permissions' ) ) {
	...
	class Branda_Permissions extends Branda_Helper {
		...
		public function ajax_save_user() {
			...
			$item = [
				'nonce' => $this->get_nonce_value( $id, 'remove' ),
				'message' => $message,
				'id' => $id,
			];
			/**
		...
		private function update_allowed_users( $users = null ) {
			...
			ub_update_option( $this->option_name_users, $users );
		...
		public function get_allowed_user_ids() {
			$permission_settings = ub_get_option_filtered( $this->option_name_users );
		...
		public function get_allowed_roles() {
			$permission_settings = ub_get_option_filtered( $this->option_name );
			$roles = !empty( $permission_settings['permissions']['user_roles'] ) ? array_keys( $permission_settings['permissions']['user_roles'] ) : [];
