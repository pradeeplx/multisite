				...
				foreach( $customer_orders as $customer_order ) {
					...
					<tr class="order" data-order_id="<?php echo esc_attr( $order_id ); ?>">
						...
						<td class="order-detail">
							...
							<?php echo '<a href="' . esc_url( $url ) . '#!/' . esc_attr( $order_id ) . '" class="um-woo-view-order um-tip-n" title="' . __( 'View order', 'um-woocommerce' ) . '"><i class="um-icon-eye"></i></a>'; ?>
							...
							if( 'completed' ==  $order->get_status() ){
								...
								$check_review = $wpdb->get_row( "SELECT * FROM  $table_post  WHERE `meta_key` = '_review_order_id' AND `meta_value` = '".$order_id."' ");

							 if( !isset($check_review->post_id)){
							 		echo '<a style="margin-left: 10px;" href="' . esc_url( $url ) . '#!/' . esc_attr( $order_id ) . '" class="um-woo-add-review um-tip-n" title="' . __( 'Review', 'um-woocommerce' ) . '"><i class="um-icon-star"></i></a>'; 
								}
