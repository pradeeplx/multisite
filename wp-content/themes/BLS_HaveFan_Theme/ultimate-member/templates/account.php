				...
				<ul>
					<?php foreach ( UM()->account()->tabs as $id => $info ) {
						if ( isset( $info['custom'] ) || UM()->options()->get( "account_tab_{$id}" ) == 1 || $id == 'general' ) { ?>
							...
							<li class="scollActive">
								<a data-tab="<?php echo esc_attr( $id )?>" href="#<?php echo $id; ?>" class="scrollAccount um-account-link <?php if ( $id == UM()->account()->current_tab ) echo 'current'; ?>">
									<?php if ( UM()->mobile()->isMobile() ) { ?>
				...
				foreach ( UM()->account()->tabs as $id => $info ) {
					...
					if ( isset( $info['custom'] ) || UM()->options()->get( 'account_tab_' . $id ) == 1 || $id == 'general' ) { ?>
						...
						<?php if($id=='information'){
							...
							?>
							<div class="scrool-tabs" id="<?php echo $id; ?>">

							   <div class="um-shadow p-20 ">
