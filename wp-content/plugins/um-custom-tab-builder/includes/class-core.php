...
class UMCTB_Core {
	...
	public function setup_post_type() {
		$labels = array(
			'name'			   => _x( 'Tabs', 'post type general name', 'um-docs' ),
			'singular_name'	  => _x( 'Tab', 'post type singular name', 'um-docs' ),
			'menu_name'		  => _x( 'Tabs', 'admin menu', 'um-docs' ),
			'name_admin_bar'	 => _x( 'Tab', 'add new on admin bar', 'um-docs' ),
			'add_new'			=> _x( 'Add New', 'tab', 'um-docs' ),
			'add_new_item'	   => __( 'Add New Tab', 'um-docs' ),
			'new_item'		   => __( 'New Tab', 'um-docs' ),
			'edit_item'		  => __( 'Edit Tab', 'um-docs' ),
			'view_item'		  => __( 'View Tab', 'um-docs' ),
			'all_items'		  => __( 'All Tabs', 'um-docs' ),
			'search_items'	   => __( 'Search Tabs', 'um-docs' ),
			'parent_item_colon'  => __( 'Parent Tabs:', 'um-docs' ),
			'not_found'		  => __( 'No tabs found.', 'um-docs' ),
			'not_found_in_trash' => __( 'No tabs found in Trash.', 'um-docs' ),
		);

		$args = array(
			'labels'			 => $labels,
			'description'		 => __( 'Description.', 'um-docs' ),
			'public'			 => false,
	...
	public function setup_metabox() {
		...
		$cmb = new_cmb2_box( array(
			'id'           => $prefix . 'metabox',
			'title'        => __( 'Tab Details', 'um-custom-tab-builder' ),
		...
		$cmb->add_field( array(
			'name' => __( 'Icon', 'um-custom-tab-builder' ),
			'id' => $prefix . 'icon',
			'type'    => 'icon_picker',
			'options' => array(
		...
		$cmb->add_field( array(
			'name'        => __( 'Slug', 'um-custom-tab-builder' ),
			'id'          => $prefix . 'slug',
			'type'        => 'text_medium',
			'description' => __( 'Choose the slug for the tab', 'um-custom-tab-builder' )
		) );

		$cmb->add_field( array(
			'name'               => __( ' Private Tab', 'um-custom-tab-builder' ),
			'id'                 => $prefix . 'private',
			'type'               => 'checkbox',
			'desc'               => __( 'Only owners of the profile can view tab.', 'um-custom-tab-builder' ),
		) );

		$cmb->add_field( array(
			'name'              => __( 'Roles with this tab', 'um-custom-tab-builder' ),
			'id'                => $prefix . 'roles_own',
			'type'              => 'multicheck',
		...
		$cmb->add_field( array(
			'name'               => __( ' Roles that can view this tab', 'um-custom-tab-builder' ),
		...
		/*
		$cmb->add_field( array(
			'name' => __( 'Position', 'um-custom-tab-builder' ),
			'id' => $prefix . ' position',
			'type' => 'text_small',
			'attributes' => array(
				'type' => 'number',
		...
		$cmb->add_field( array(
			'name' => __( 'Parent Tab', 'um-custom-tab-builder' ),
			'id' => $prefix . 'parent_tab',
			'type' => 'select',
			'options' => $this->get_parent_tabs(),
		) );
		*/
		$type_options = array();
		$type_options['profile'] = __( 'Profile', 'um-custom-tab-builder' );
		if ( class_exists( 'UM_Groups' ) ) {
			$type_options['group'] = __( 'Group', 'um-custom-tab-builder' );
		}
		$cmb->add_field( array(
			'name'    => __( 'Tab Type', 'um-custom-tab-builder' ),
			'id'      => $prefix . 'tab_type',
			'type'    => 'select',
			'default' => 'profile',
			'options' => $type_options,
			'description' => __( 'What type of tab is being created? Groups extension needed to be used.', 'um-custom-tab-builder' )
		) );

		$cmb->add_field( array(
			'name'    => __( 'Content Type', 'um-custom-tab-builder' ),
			'id'      => $prefix . 'content_type',
			'type'    => 'select',
			'options' => $this->get_content_types(),
			'description' => __( 'What type of content will be placed in tab? UM Form not available for group tab types.', 'um-custom-tab-builder' )
		) );


		$cmb->add_field( array(
			'name'    => __( 'UM Form', 'um-custom-tab-builder' ),
			'id'      => $prefix . 'type_form',
			'type'    => 'select',
			'classes' => 'um-ctb--hide',
			'options' => $this->get_um_profile_forms(),
			'description' => __( 'Choose the UM Form that users will edit and see.', 'um-custom-tab-builder' )

		) );

		$cmb->add_field( array(
			'name'    => __( 'Shortcode', 'um-custom-tab-builder' ),
			'id'      => $prefix . 'type_shortcode',
			'type'    => 'textarea',
			'classes' => 'um-ctb--hide',
			'description' => __( 'Add shortcodes and shorttags here. Use UM Form type for UM Forms.', 'um-custom-tab-builder' )
		) );

		$cmb->add_field( array(
			'name'    => __( 'Content', 'um-custom-tab-builder' ),
			'id'      => $prefix . 'type_content',
			'type'    => 'wysiwyg',
			'classes' => 'um-ctb--hide',
			'description' => __( 'Add html content, shortcodes and shorttags here.', 'um-custom-tab-builder' )
		) );

		/*
		$cmb->add_field( array(
			'name'    => __( 'Code', 'um-custom-tab-builder' ),
			'id'      => $prefix . 'type_code',
			'type'    => 'textarea_code',
			'classes' => 'um-ctb--hide',
		) );
	...
	public function get_content_types() {
		$content_types = array(
			'shortcode' => __( 'Shortcode', 'um-custom-tab-builder' ),
			'form'      => __( 'UM Form', 'um-custom-tab-builder' ), 
			'content'   => __( 'Content', 'um-custom-tab-builder' ), 
		);
	...
	public function get_um_profile_forms() {
		global $wpdb;
		$forms = $wpdb->get_results( $wpdb->prepare( "SELECT ID, post_title FROM {$wpdb->posts} WHERE post_type='%s' AND post_status='publish'", 'um_form' ) );
	...
	public function add_admin_assets() {
		...
		wp_enqueue_script( 'um_ctb_admin', um_ctb()->url( 'assets/js/um_ctb.js' ), array( 'jquery' ), UM_Custom_Tab_Builder::VERSION );
		if ( defined( 'um_url' ) ) {
			wp_enqueue_style( 'um_ctb_admin-um-fonts-styles', um_url .  'assets/css/um-fonticons-fa.css' );
		}
		wp_enqueue_style( 'um_ctb_admin-styles', um_ctb()->url( 'assets/css/um_ctb-admin.css' ) );
	}

	public function wpcf7_before_send_mail( $form = array() ) {
		if ( ! isset( $_POST['_um_profile_id'] ) ) {
			return;
		}

		$user_id = absint( $_POST['_um_profile_id'] );
