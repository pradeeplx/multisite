...
if ( ! function_exists( 'themify_builder_activate' ) ) {
	...
	function themify_builder_activate() {
		...
		/**
		 * register the custom post types added by Builder and flush permalinks,
		 * this fixes 404 errors.
		 */
		$path = trailingslashit( plugin_dir_path( __FILE__ ) );
		if ( ! class_exists( 'CPT' ) ) {
			require_once( $path . 'includes/libraries/CPT.php' );
		}
		require_once( $path . 'classes/class-themify-builder-layouts.php' );
		require_once( $path . 'classes/class-themify-global-styles.php' );
		require_once( $path . 'classes/class-themify-custom-fonts.php' );
		Themify_Builder_Layouts::register_layout_post_type();
		Themify_Builder_Layouts::register_layout_part_post_type();
		Themify_Global_Styles::register_global_style();
		Themify_Custom_Fonts::register_cpt();
		flush_rewrite_rules();
...
function themify_builder_plugin_init() {
	...
	if ( Themify_Builder_Model::builder_check() ) {
		...
		$Themify_Global_Styles = new Themify_Global_Styles();
		$Themify_Custom_Fonts = new Themify_Custom_Fonts();
