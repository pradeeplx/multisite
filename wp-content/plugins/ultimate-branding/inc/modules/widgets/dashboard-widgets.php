...
if ( ! class_exists( 'Branda_Dashboard_Widgets' ) ) {
	...
	class Branda_Dashboard_Widgets extends Branda_Helper {
		...
		public function upgrade_options() {
			 ...
			 */
			$option = ub_get_option_filtered( 'ub_remove_wp_dashboard_widgets' );
			if (
			 ...
			 */
			$options = ub_get_option_filtered( 'wpmudev_dashboard_text_widgets_options' );
			if ( ! empty( $options ) ) {
				$data = array();
		...
		public function delete_available_widget( $id ) {
			$value = ub_get_option_filtered( $this->available_widgets );
			if ( isset( $value[ $id ] ) ) {
				unset( $value[ $id ] );
