...
class WCFMmp_Gateway_Stripe_Split extends WC_Payment_Gateway {
				...
				if( $pvendor_id && ( $pvendor_id == $vendor_id ) ) {
					...
					if( $WCFMmp->wcfmmp_vendor->is_vendor_get_tax( $vendor_id ) ) {
						...
						if( apply_filters( 'wcfmmp_is_allow_commission_on_tax', false ) ) {
							$tax_cost = $WCFMmp->wcfmmp_commission->wcfmmp_generate_commission_cost( $tax_cost, apply_filters( 'wcfmmmp_tax_commission_rule', $commission_rule ) );
						}
			...
			foreach ($shipping_items as $shipping_item_id => $shipping_item) {
				...
				if( $shipping_vendor_id > 0 && ( $shipping_vendor_id == $vendor_id ) ) {
					...
					if( apply_filters( 'wcfmmp_is_allow_commission_on_shipping', false ) ) {
						$shipping_cost = $WCFMmp->wcfmmp_commission->wcfmmp_generate_commission_cost( $shipping_cost, apply_filters( 'wcfmmmp_shipping_commission_rule',  $commission_rule ) );
						$shipping_tax  = $WCFMmp->wcfmmp_commission->wcfmmp_generate_commission_cost( $shipping_tax, apply_filters( 'wcfmmmp_shipping_commission_rule',  $commission_rule ) );
					}
	...
	public function update_customer_obj( $user, $postData ) {
		try {
			...
			wcfm_log("Error update customer cards with Stripe: " . $ex->getMessage());
						...
						if( $vendor_id && ( $charge_type == 'transfers_charges' ) ) {
							...
							if( $vendor_token ) {
								...
								if( $vendor_trasfer_id ) {
									...
									wcfm_log( "Stripe Split Pay transfer reversal successful for #{$refund_id}. Vendors ID => {$vendor_id} Transaction ID => {$vendor_trasfer_id} Stripe refund ID => " . $refund->id );
								}
							}
						}
					} catch( Exception $e ) {
						 wcfm_log( "Stripe Split Pay transfer reversal error: " . $e->getMessage() );
					}
