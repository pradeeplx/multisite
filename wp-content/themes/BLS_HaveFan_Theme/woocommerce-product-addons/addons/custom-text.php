...
if( !empty( $field_name_array) ){
	$cs_product_id = $field_name_array[0];
	$cs_field_val = $field_name_array[1];
	$cs_field_index = $field_name_array[2];
	$cs_product_addon = get_post_meta( $cs_product_id, '_product_addons', true);
 
	if(isset($cs_product_addon[$cs_field_index]) ){
		$cs_product_descr = $cs_product_addon[$cs_field_index]['description'];
	}
