...
class WC_Booking_Cart_Manager {
	...
	public function get_item_data( $other_data, $cart_item ) {
		...
		switch ( $cart_item['booking']['_duration_unit'] ) {
			case 'day':
			case 'month':
				$format     = 'day' === $cart_item['booking']['_duration_unit'] ? get_option( 'date_format' ) : 'F Y';
				$start_date = date_i18n( $format, $cart_item['booking']['_start_date'] );
				$end_date   = date_i18n( $format, $cart_item['booking']['_end_date'] );

				$cart_item['booking']['date'] = $start_date;

				if ( $start_date !== $end_date ) {
					$cart_item['booking']['date'] .= ' - ';
					$cart_item['booking']['date'] .= $end_date;
				}
				break;
			case 'hour':
			case 'minute':
				$cart_item['booking']['date']  = date_i18n( get_option( 'date_format' ), $cart_item['booking']['_start_date'] );
				$cart_item['booking']['date'] .= ' / ';
				$cart_item['booking']['date'] .= date_i18n( get_option( 'time_format' ), $cart_item['booking']['_start_date'] );
				$cart_item['booking']['date'] .= ' - ';
				$cart_item['booking']['date'] .= date_i18n( get_option( 'time_format' ), $cart_item['booking']['_end_date'] );
				break;
		}

		foreach ( $cart_item['booking'] as $key => $value ) {
			if ( substr( $key, 0, 1 ) !== '_' && ! in_array( $key, array( 'time' ), true ) ) {
				$other_data[] = array(
