...
define( 'UM_GALLERY_LITE_VERSION', '1.0.5' );
	...
	class UM_Gallery {
		...
		**/
		...
		public function load_plugin_textdomain() {
			...
			$loaded = load_plugin_textdomain( 'um-gallery', false, '/languages/' );

			if ( ! $loaded ) {
				$loaded = load_muplugin_textdomain( 'um-gallery', '/languages/' );
			}

			if ( ! $loaded ) {
				$loaded = load_theme_textdomain( 'um-gallery', get_stylesheet_directory() . '/languages/' );
			}

			if ( ! $loaded ) {
				$locale = apply_filters( 'plugin_locale', get_locale(), 'um-gallery-pro' );
				$mofile = dirname( __FILE__ ) . '/languages/um-gallery-' . $locale . '.mo';
				load_textdomain( 'um-gallery', $mofile );
			}
		}
...
if ( ! function_exists( '_um_gallery_pro_activate' ) ) {
	function _um_gallery_pro_activate( $network_wide ) {
		...
		update_option( 'um_gallery_version', '1.0.4.1' );
	}
}
