...
if ( ! class_exists( 'Brenda_Maintenance' ) ) {
	...
	class Brenda_Maintenance extends Branda_Helper {
		...
		public function output() {
			...
			$args['head'] .= $result['head'];
			$args['head'] .= $this->enqueue( 'css/front-end/maintenance.css' );
			/**
			 * Common: body
			 */
			$args['css'] .= $this->common_css_body();
			/**
			 * Common: Document
			 */
			$args['css'] .= $this->common_css_document();
			/**
			 * Common Background
			 *
			 * @since 2.3.0
			 */
			$args['head'] .= $this->css_background_common( 'html', false );
			/**
			 * Logo
			 */
