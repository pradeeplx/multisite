...
function get_ub_activated_modules( $mode = 'normal' ) {
	...
	if ( empty( $branda_modules ) ) {
		$branda_modules = ub_get_option( 'ultimatebranding_activated_modules', array(), $mode );
	}
	/**
	 * For mode 'raw' do nor use filters.
	 *
...
function update_ub_activated_modules( $data ) {
	global $branda_modules;
	$branda_modules = $data;
	ub_update_option( 'ultimatebranding_activated_modules', $branda_modules );
}
 ...
 * @return bool
 */
function branda_is_wpmudev_member() {
