	...
	class Branda_Site_Status_Pages extends Branda_Helper {
		...
		private function update_files( $value ) {
			...
			foreach ( $this->error_files as $slug => $f ) {
				...
				$args['head'] .= $this->enqueue( 'css/ms-site-check.css' );
				$args['head'] .= $this->css_background_common( 'body', false );
				 ...
				 * BODY
				 */
				$args['css'] .= $this->common_body_css();
				/**
				 * .page
				 */
				$args['css'] .= $this->common_document_css( '.page' );
				/**
				 * h1
				 */
				$value = $this->get_value( 'colors', 'message_title' );
				$args['css'] .= sprintf( '.page h1{%s}', $this->css_color( $value ) );
				/**
				 * content
				 */
				$value = $this->get_value( 'colors', 'message_description' );
				$args['css'] .= sprintf( '.page .content{%s}', $this->css_color( $value ) );
				/**
				 * Custom CSS
				 *
				 * @since 3.0.0
